...
public class GamePlay {
	...
	class MyThread extends Thread{
    	...
    	public void run() {
    	     timeline= new Timeline(new KeyFrame(Duration.millis(350), ev -> {
    	    	...
    	    	if(interval<0) {
    	    		if(horse.isBackHome()) {
    	    			int currentScore=players[horse.getPlayerIndex()].getScore();
    	    			players[horse.getPlayerIndex()].setScore(currentScore+to);
    	    		}
    	    		...
    	    		if(d1.getValue()!=d2.getValue())
	 	    		...
	 	    		if(currentHorse!=null) {
	 	    			if(currentHorse.getPlayerIndex()!=horse.getPlayerIndex()) {
		 	    			...
		 	    			if(homes[currentHorse.getPlayerIndex()].getChildren().contains(currentHorse.getImgHorse()))
		 	    				homes[currentHorse.getPlayerIndex()].getChildren().remove(currentHorse.getImgHorse());
	 	    		...
	 	    		if(horse.isOnHouse())
	 	    			horse.setBackHome(false);
	 	    		int lposition=horse.getPosition();
	 	    		int limit=horse.getLimit();
	 	    		if(lposition==limit+5||((lposition==limit+4)&&(horses[limit+3]!=null))
	 	    				||((lposition==limit+4)&&(horses[limit+5]!=null)) 
	 	    				||((lposition==limit+3)&&(horses[limit+4]!=null)))
        	    	...
        	    	if(!this.horse.isBackHome()) {
        	    		if((horsePosition==1||horsePosition==19||horsePosition==37||horsePosition==55)&&(this.horse.isOnHouse()==false))
        	    			this.horse.setPosition(horsePosition+6);
        	    	}
        	    	horsePosition=this.horse.getPosition();
    ...
    public void rotatedDice2() {
        ...
        rt.setOnFinished(actionEvent -> {
            ...
            for(int i=0;i<4;i++) {
    			...
    			boolean isFirstStep=checkFirstStep(horse);
    			if(result[0]==false&&result[1]==false&&result[2]==false&&isFirstStep==false) {
    				increaseCurrentPlayerIndex();
    				lb_currentplayername.setText(playerNames[currentPlayer]);
    ...
    private boolean checkFirstStep(Horse horse) {
		if(horse.isOnHouse()) {
			if(d1.getValue()==6||d2.getValue()==6) {
				sound.playOutNestSound();
				horse.setOnHouse(false);
				horse.setPosition(horse.getPosition()+12*horse.getPlayerIndex()+(horse.getPlayerIndex()+1)*6);
    	...
    	for(Horse h:horses) {
    		...
    		if(horse.isBackHome()) {
    			...
    			if(isThroughHouse!=-1) {
    				max=max+6;
    				for(int i=currentPosition;i<max;i++) {
    					if((i-isThroughHouse)>0&&(i-isThroughHouse)<7)
    						continue;
//    					if(horses[i]!=null) {
//    						result[0]=false;
//                			break;
//    					}
        		...
        		if((currentPosition<horse.getLimit()&&max>horse.getLimit())||(horse.getLimit()==1&&max>72)){
        			result[0]=false;
        			break;
        		}
        		if(max>71) {
...
//    					}
    	...
    	for(Horse h:horses) {
    		if(h==null)
    			continue;
    		if(horse.isBackHome()) {
    			int max=currentPosition+number1+number2;
    			if(max>71)
    				max=max%72;
    			if(max>horse.getLimit()+5)
    			{
